...
SELECT BRANCH , SUM(COGS) AS TOTAL_SALES FROM SALES GROUP BY BRANCH ORDER BY TOTAL_SALES;
...
-- 2) Product Line Performance in the Lowest Sales Branch- a) Average Sales of Each Product Line in All Branches
 SELECT PRODUCT_LINE,AVG(COGS) AS AVG_SALES 
           FROM SALES 
           GROUP BY PRODUCT_LINE
...
SELECT PRODUCT_LINE,AVG_SALES_B,AVG_SALES_ABC
FROM 
(SELECT 
	PRODUCT_LINE,
	AVG(COGS) AS AVG_SALES_B
FROM SALES 
WHERE BRANCH='B'
GROUP BY PRODUCT_LINE) ASB
JOIN
(SELECT PRODUCT_LINE,AVG(COGS) AS AVG_SALES_ABC
FROM SALES 
GROUP BY PRODUCT_LINE) ASABC
           ON ASB.PRODUCT_LINE=ASABC.PRODUCT_LINE
